...
const PublicDisplay = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            setProcessingOrders(data.filter((o: any) => o.status === 'PREPARING'));
        } catch (err) {
    ...
    return (
        <div className="h-screen bg-slate-900 text-white p-10 flex flex-col font-sans" dir="rtl">
            ...
            <div className="grid grid-cols-2 gap-10 flex-1">
                <div className="bg-emerald-900/20 border-4 border-emerald-500 rounded-[3rem] p-10 text-center shadow-2xl shadow-emerald-500/10">
                    <h2 className="text-4xl font-bold text-emerald-400 mb-12 flex items-center justify-center gap-4">
                        جاهز للاستلام ✅
                    </h2>
                    <div className="grid grid-cols-2 gap-8">
                        {readyOrders.map(order => (
                            <div key={order.id} className="text-9xl font-black animate-pulse text-white drop-shadow-lg">
                                {order.queueNumber}
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-blue-900/10 border-4 border-blue-500/30 rounded-[3rem] p-10 text-center">
